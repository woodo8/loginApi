...
class DiagnosticNodeInternal {
    ...
    recordSuccessfulNetworkCall(startTimeUTCInMs, requestContext, pipelineResponse, substatus, url) {
        ...
        const gatewayRequest = {
            ...
            correlateActivityId: requestContext.headers[Constants$1.HttpHeaders.CorrelatedActivityId],
            startTimeUTCInMs,
    ...
    recordFailedNetworkCall(startTimeUTCInMs, requestContext, retryAttemptNumber, statusCode, substatusCode, responseHeaders) {
        ...
        this.diagnosticCtx.recordFailedAttempt({
            activityId: responseHeaders[Constants$1.HttpHeaders.ActivityId],
            correlatedActivityId: requestContext.headers[Constants$1.HttpHeaders.CorrelatedActivityId],
            startTimeUTCInMs,
            durationInMs: getCurrentTimestampInMs() - startTimeUTCInMs,
            statusCode,
...
function mergeHeaders(headers, toBeMergedHeaders) {
    ...
    if (Constants$1.HttpHeaders.CorrelatedActivityId in toBeMergedHeaders) {
        headers[Constants$1.HttpHeaders.CorrelatedActivityId] =
            toBeMergedHeaders[Constants$1.HttpHeaders.CorrelatedActivityId];
    }
}
...
class FeedResponse {
    ...
    get correlatedActivityId() {
        return this.headers[Constants$1.HttpHeaders.CorrelatedActivityId];
    }
    get indexMetrics() {
...
class ChangeFeedForEpkRange {
    ...
    async getFeedResponse(feedRange, diagnosticNode) {
        ...
        if (feedRange.continuationToken) {
            ...
            feedOptions.initialHeaders[Constants$1.HttpHeaders.IfNoneMatch] =
                Constants$1.ChangeFeedIfNoneMatchStartFromNowHeader;
        }
        if (this.startTime) {
...
async function getHeaders({ clientOptions, defaultHeaders, verb, path, resourceId, resourceType, options = {}, partitionKeyRangeId, useMultipleWriteLocations, partitionKey, }) {
    ...
    if (options.useAllVersionsAndDeletesFeed) {
        ...
        headers[Constants$1.HttpHeaders.A_IM] = exports.ChangeFeedMode.AllVersionsAndDeletes;
        headers[Constants$1.HttpHeaders.ChangeFeedWireFormatVersion] =
            Constants$1.AllVersionsAndDeletesChangeFeedWireFormatVersion;
    }
    if (options.useIncrementalFeed || options.useLatestVersionFeed) {
        headers[Constants$1.HttpHeaders.A_IM] = exports.ChangeFeedMode.LatestVersion;
    }
    ...
    if (options.bypassIntegratedCache) {
        headers[Constants$1.HttpHeaders.DedicatedGatewayPerRequestBypassCache] =
...
async function execute({ diagnosticNode, retryContext = { retryCount: 0 }, retryPolicies, requestContext, executeRequest, }) {
    ...
    return addDignosticChild(async (localDiagnosticNode) => {
        ...
        const correlatedActivityId = requestContext.headers[Constants$1.HttpHeaders.CorrelatedActivityId];
        try {
            ...
            if (correlatedActivityId) {
                response.headers[Constants$1.HttpHeaders.CorrelatedActivityId] = correlatedActivityId;
            }
            return response;
        }
        catch (err) {
            ...
            if (correlatedActivityId) {
                headers[Constants$1.HttpHeaders.CorrelatedActivityId] = correlatedActivityId;
            }
...
function getUserAgent(suffix) {
    const ua = `${userAgentDetails()} ${Constants$1.SDKName}/${Constants$1.SDKVersion}`;
    if (suffix) {
        return ua + " " + suffix;
    }
    return ua;
...
// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.
const diagnosticLevelFromEnv = process$1.env[Constants$1.CosmosDbDiagnosticLevelEnvVarName];
